...
def runBOCS(n_vars, n_init, eval_budget, sim_anneal, lamb, job):
	...
	inputs['model']    = lambda x: float(subprocess.check_output(cmd + [str(job)] + [str(i) for i in x], universal_newlines = True))
