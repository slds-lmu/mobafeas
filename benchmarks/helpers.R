...
makeMBOResult = function(design, ninit = NULL, p) {
  ...
  ctrl = setMBOControlTermination(ctrl, max.evals = 1L)

  # --- put everything into an MBO result object
  result = mbo(obj, design = design, control = ctrl)
  
  if (!is.null(ninit)){
    dob = result$opt.path$env$dob
    result$opt.path$env$dob = c(rep(0L, ninit), 1:(length(dob) - ninit))
  }
